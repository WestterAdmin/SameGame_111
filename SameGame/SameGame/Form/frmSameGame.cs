...
namespace SameGame
{
    public partial class frmSameGame : Form
    {
        ...
        private string GetRandomImage()
        {
            ...
            string file = string.Format("FILE_LOG_{0}", DateTime.Now.ToString("yyyyMMdd"));
            if (!Directory.Exists(pathCurrentProject + "\\" + file))
            {
                Directory.CreateDirectory(pathCurrentProject + "\\" + file);
            }
            using (StreamWriter w = File.AppendText(string.Format("{0}\\{1}\\{2}_{3}.txt", pathCurrentProject, file, "log", DateTime.Now.Hour.ToString())))
            {
                Log(index, path, w);
            }
            return path; 
        ...
        private void Log(int index, string logMessage, StreamWriter txtWriter)
        {
            ...
            {
                // if(Path.co)
                txtWriter.Write(string.Format("++++++++++++++++++++++{0}++++++++++++++++++++++",DateTime.Now.ToString("yyyy.MM.dd.HH.mm.ss")));
                txtWriter.Write("\r\nLog Entry : ");
        ...
        private void Pic_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            // tìm kiếm những điểm bên trên
            // tìm kiếm những điểm bên dưới
            getBallBottom(ball);
            // tìm kiếm điểm bên trái
            // tìm kiếm điểm bên phải
        ...
        private void getBallBottom(PictureBox BallIndex)
        {
